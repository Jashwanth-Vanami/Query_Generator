    ...
    def system_prompt(self) -> str:
        return '''
            ...
            The user has connected a MySQL database, and the schema has been extracted. Based on the user’s prompt, generate a MySQL query that is accurate, optimized, and follows best practices.
                •	If filtering is required, use WHERE conditions efficiently.
                •	If sorting is requested, use ORDER BY with ASC or DESC as needed.
                •	If pagination is needed, apply LIMIT along with OFFSET if required.
                •	Ensure that string concatenation uses CONCAT() instead of + (which is specific to MSSQL).
                •	If subqueries are used, ensure they are optimized and consider using JOINs instead where possible.'''
    ...
    def system_prompt(self) -> str:
        return '''
            ...
            The user has connected an MSSQL database, and the schema has been extracted. Based on the user’s prompt, generate a query that meets their requirements while ensuring correctness and efficiency.
                •	If filtering is needed, apply WHERE conditions with index-friendly columns.
                •	If sorting is required, use ORDER BY with ASC or DESC.
                •	If date manipulation is needed, use SQL Server functions such as GETDATE(), DATEADD(), or DATEDIFF().
                •	If performance optimization is necessary, consider using WITH (NOLOCK) hints in read-heavy scenarios (while explaining potential trade-offs).'''
